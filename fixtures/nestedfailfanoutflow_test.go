...
func TestNestedfailfanoutflow(t *testing.T) {
	...
	graph := workflow.NewGraph("nestedfailfanoutflow.verify:428/nested")
	...
	proxy.HandleTask("nestedfailfanoutflow.verify:428/task-a", func(ctx context.Context, f *workflow.Flow, baggage any) error {
		f.Set("outers", []int{0, 1, 2})
		return nil
	})
	proxy.HandleTask("nestedfailfanoutflow.verify:428/task-o", func(ctx context.Context, f *workflow.Flow, baggage any) error {
		f.Set("inners", []int{0, 1, 2})
		f.SetInt("currentOuter", f.GetInt("outerItem"))
		return nil
	})
	proxy.HandleTask("nestedfailfanoutflow.verify:428/task-i", func(ctx context.Context, f *workflow.Flow, baggage any) error {
		mu.Lock()
		innerStarts++
		mu.Unlock()
	...
	proxy.HandleTask("nestedfailfanoutflow.verify:428/join-i", func(ctx context.Context, f *workflow.Flow, baggage any) error {
		mu.Lock()
		joinIRuns++
		mu.Unlock()
		return nil
	})
	proxy.HandleTask("nestedfailfanoutflow.verify:428/join-o", func(ctx context.Context, f *workflow.Flow, baggage any) error {
		mu.Lock()
		joinORuns++
		mu.Unlock()
		return nil
	...
	flowKey, err := eng.Create(ctx, "nestedfailfanoutflow.verify:428/nested", nil, nil, nil)
