...
func TestStaterefsflow(t *testing.T) {
	...
	t.Run("carried_document_survives_fanout_and_fanin", func(t *testing.T) {
		...
		assert.Equal(float64(docLen), stateVal(outcome.State, "pdfLen"))
		assert.Equal(6.0, stateVal(outcome.State, "pageCount"), "a task's own field must not be eaten by a same-named forEach bookkeeping key")
		...
		assert.Equal(docLen, len(stateVal(outcome.State, "pdf").(string)))
		...
		for _, s := range steps {
			...
			assert.NoError(err)
			doc, ok := stateVal(step.State, "pdf").(string)
	...
	t.Run("continue_carries_the_document_into_the_next_turn", func(t *testing.T) {
		...
		assert.Equal(float64(docLen), stateVal(turn2.State, "pdfLen"))
		assert.Equal(docLen, len(stateVal(turn2.State, "pdf").(string)))
	})
	...
	t.Run("fork_clones_a_self_contained_tree", func(t *testing.T) {
		...
		assert.Equal(float64(docLen), stateVal(forkOutcome.State, "pdfLen"))
		assert.Equal(6.0, stateVal(forkOutcome.State, "pageCount"))
		...
		assert.Equal(docLen, len(stateVal(origin.State, "pdf").(string)))
	})
	...
	t.Run("a_task_can_overwrite_and_delete_a_refd_field", func(t *testing.T) {
		...
		assert.NoError(err)
		assert.Equal(workflow.StatusCompleted, outcome.Status)
		assert.Equal(false, stateVal(outcome.State, "pdfPresent"), "a deleted field must not be resurrectable through its ref")
		assert.Equal(0.0, stateVal(outcome.State, "pdfLen"))
		assert.False(outcome.State.Has("pdf"))
	})
}
