...
func TestRetryfanoutflow(t *testing.T) {
	...
	proxy.HandleTask("retryfanoutflow.verify:428/enter", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("retryfanoutflow.verify:428/increment", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	proxy.HandleTask("retryfanoutflow.verify:428/join", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	...
	t.Run("ordered_despite_random_retries", func(t *testing.T) {
		...
		outcome, err := eng.Run(ctx, "retryfanoutflow.verify:428/retry-fan-out", initialState, nil, nil)
		assert.NoError(err)
