...
func TestReducerflow(t *testing.T) {
	...
	graph := workflow.NewGraph("reducerflow.verify:428/reducer")
	...
	proxy.HandleTask("reducerflow.verify:428/task-a", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("reducerflow.verify:428/task-b", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("total", 10)
		f.SetStrings("tags", []string{"b"})
		f.SetStrings("seen", []string{"x"})
		return nil
	})
	proxy.HandleTask("reducerflow.verify:428/task-c", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("total", 20)
		f.SetStrings("tags", []string{"c"})
		f.SetStrings("seen", []string{"y", "x"})
		return nil
	})
	proxy.HandleTask("reducerflow.verify:428/task-d", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("total", 30)
		f.SetStrings("tags", []string{"d"})
		f.SetStrings("seen", []string{"z"})
		return nil
	})
	proxy.HandleTask("reducerflow.verify:428/task-e", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	t.Run("sum_list_and_set_reducers_apply", func(t *testing.T) {
		...
		outcome, err := eng.Run(ctx, "reducerflow.verify:428/reducer", nil, nil, nil)
		assert.NoError(err)
