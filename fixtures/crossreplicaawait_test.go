...
func TestCrossReplicaAwait(t *testing.T) {
	...
	proxy1.HandleTask("crossreplica.verify:428/work", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	proxy2.HandleTask("crossreplica.verify:428/work", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	flowKey, err := eng1.Create(ctx, "crossreplica.verify:428/flow", nil, nil, nil)
