...
// (warmup then measure) with the SAME b.Name(); reusing it as the NewEngineUnderTest key would collide with
...
// or in-memory SQLite when unset), wired to a linear chain graph of benchChainLen trivial tasks. It silences
// the engine's logger (NewEngineUnderTest defaults to stderr at Error, and per-step logging would dominate
// the timing) and registers cleanup via b.Cleanup. Returns the engine and the workflow URL to Run.
...
func benchEngine(b *testing.B, numShards int) (*engine.Engine, string) {
	...
	// own isolated, auto-dropped database set.
	eng := engine.NewEngineUnderTest(fmt.Sprintf("%s-%d", b.Name(), benchSeq.Add(1)))
	// Silence the engine: NewEngineUnderTest logs to stderr at Error, and a benchmark's per-iteration
	// logging would dominate the measurement.
	if err := eng.SetLogger(slog.New(slog.DiscardHandler)); err != nil {
		b.Fatal(err)
	}
