...
func TestInterruptpayloadflow(t *testing.T) {
	...
	graph := workflow.NewGraph("interruptpayloadflow.verify:428/interrupt")
	...
	proxy.HandleTask("interruptpayloadflow.verify:428/setup", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetString("prompt", "choose")
		return nil
	})
	proxy.HandleTask("interruptpayloadflow.verify:428/ask", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	t.Run("payload_is_separate_from_state", func(t *testing.T) {
		...
		flowKey, err := eng.Create(ctx, "interruptpayloadflow.verify:428/interrupt", nil, nil, nil)
		...
		graph, err := proxy.LoadGraph(ctx, "interruptpayloadflow.verify:428/interrupt", nil)
		if !assert.NoError(err) {
			return
		}
