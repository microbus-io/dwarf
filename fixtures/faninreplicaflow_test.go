...
func TestFanInReplicaflow(t *testing.T) {
	...
	for i, k := range keys {
		...
		assert.Equal(60.0, stateVal(out.State, "finalSum"))
		assert.Equal([]string{"b", "c", "d"}, sortedStateStrings(stateVal(out.State, "finalList")))
		assert.Equal([]string{"x", "y", "z"}, sortedStateStrings(stateVal(out.State, "finalSet")))
