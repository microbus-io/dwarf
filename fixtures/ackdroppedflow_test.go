...
func TestAckdroppedflow(t *testing.T) {
	...
	ackGraph := workflow.NewGraph("ackdroppedflow.verify:428/ack-dropped")
	...
	echoGraph := workflow.NewGraph("ackdroppedflow.verify:428/echo")
	...
	proxy.HandleTask("ackdroppedflow.verify:428/park", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	proxy.HandleTask("ackdroppedflow.verify:428/ping", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		pingHits.Add(1)
		return nil
	})
