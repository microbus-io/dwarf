...
func TestForkOfForkflow(t *testing.T) {
	...
	assert.Equal("x-done", stateVal(fork2Out.State, "xOut")) // X's output survived from the kept (cloned) branch
	assert.Equal("y-done", stateVal(fork2Out.State, "yOut")) // Y's output from the re-run branch, merged at J
