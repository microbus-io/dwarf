...
func TestSleepflow(t *testing.T) {
	...
	proxy.HandleTask("sleepflow.verify:428/task-a", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("sleepflow.verify:428/task-b", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.Sleep(f.GetDuration("sleepFor"))
		f.SetBool("marked", true)
		return nil
	})
	proxy.HandleTask("sleepflow.verify:428/task-c", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
