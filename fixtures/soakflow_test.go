...
func TestSoakflow(t *testing.T) {
	...
	soakGraph := workflow.NewGraph("soakflow.verify:428/soak")
	...
	innerGraph := workflow.NewGraph("soakflow.verify:428/inner")
	...
	proxy.HandleTask("soakflow.verify:428/seed", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	proxy.HandleTask("soakflow.verify:428/fan-a", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("soakflow.verify:428/work", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("work", 1)
		return nil
	})
	proxy.HandleTask("soakflow.verify:428/collect", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("soakflow.verify:428/loop", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		left := f.GetInt("loopsLeft") - 1
		f.SetInt("loopsLeft", left)
		if left > 0 {
			f.Goto("soakflow.verify:428/loop")
		}
		return nil
	})
	proxy.HandleTask("soakflow.verify:428/sub", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	proxy.HandleTask("soakflow.verify:428/boom-r", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return errors.New("soak boom (recoverable)")
	})
	proxy.HandleTask("soakflow.verify:428/recover", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("soakflow.verify:428/boom-f", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return errors.New("soak boom (fatal)")
	})
	proxy.HandleTask("soakflow.verify:428/join", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("soakflow.verify:428/inner-entry", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
