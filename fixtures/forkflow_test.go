...
func failedCellKeyByIndex(t *testing.T, eng *engine.Engine, flowKey string, idx int) string {
	...
	for _, s := range hist {
		...
		switch v := stateVal(full.State, "itemIndex").(type) {
		case float64:
			if int(v) == idx {
				return s.StepKey
			}
...
func TestForkflow_LinearWithOverride(t *testing.T) {
	...
	assert.Equal("orig", stateVal(outcome.State, "bSaw"))
	...
	assert.Equal("forked", stateVal(forkOutcome.State, "bSaw"))
	assert.Equal(true, stateVal(forkOutcome.State, "done"))
	...
	assert.Equal("orig", stateVal(origAfter.State, "bSaw"))
}
...
func TestForkflow_ContinueExcludesFork(t *testing.T) {
	...
	assert.NoError(err)
	assert.Equal(workflow.StatusCompleted, outcome.Status)
	assert.Equal("origin", stateVal(outcome.State, "tag"))
	...
	assert.NoError(err)
	assert.Equal(workflow.StatusCompleted, forkOutcome.Status)
	assert.Equal("forked", stateVal(forkOutcome.State, "tag"))
	...
	assert.Equal("origin", stateVal(contOutcome.State, "tag"))
}
...
func TestForkflow_FanoutForkOfFork(t *testing.T) {
	...
	assert.Equal(true, stateVal(fork2Outcome.State, "joined"))
}
...
func TestForkflow_AtSubgraphCaller(t *testing.T) {
	...
	assert.NoError(err)
	assert.Equal(workflow.StatusCompleted, outcome.Status)
	assert.Equal("Y(orig)", stateVal(outcome.State, "subResult"))
	...
	assert.Equal("Y(forked)", stateVal(forkOutcome.State, "subResult"))
	...
	assert.NoError(err)
	assert.Equal(workflow.StatusCompleted, origAfter.Status)
	assert.Equal("Y(orig)", stateVal(origAfter.State, "subResult"))
}
