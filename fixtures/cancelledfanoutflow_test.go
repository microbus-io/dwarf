...
func TestCancelledfanoutflow(t *testing.T) {
	...
	branch := func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		executed.Add(1)
		select {
	...
	proxy.HandleTask("cancelledfanoutflow.verify:428/source", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	...
	proxy.HandleTask("cancelledfanoutflow.verify:428/j", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
