...
func TestDuplicateEdge_WhenFanOutWidthTracksMatchingClauses(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Logf("gate=%d workRuns=%d total=%v", tc.gate, workRuns.Load(), stateVal(outcome.State, "total"))
			assert.Equal(tc.wantWorkRuns, workRuns.Load(), "Work runs once per matching `when` clause")
			if tc.wantTotal == nil {
				assert.Nil(stateVal(outcome.State, "total"), "no branch ran, so the reducer produced nothing")
			} else {
				assert.Equal(tc.wantTotal, stateVal(outcome.State, "total"), "`add` sums exactly one delta per branch")
			}
		})
	}
}
...
func TestDuplicateEdge_TrunkSourceIntoFanInIsDegenerate(t *testing.T) {
	...
	assert.NoError(err)
	t.Logf("status=%v joinRuns=%d total=%v", outcome.Status, joinRuns.Load(), stateVal(outcome.State, "total"))
	assert.Equal(workflow.StatusCompleted, outcome.Status)
