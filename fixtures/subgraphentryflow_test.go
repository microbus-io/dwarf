...
func TestSubgraphentryflow(t *testing.T) {
	...
	proxy.HandleTask("subgraphentryflow.verify:428/task-inner", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetString("innerResult", "inner")
		return nil
	})
	proxy.HandleTask("subgraphentryflow.verify:428/run-inner", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	proxy.HandleTask("subgraphentryflow.verify:428/task-tail", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetString("finalResult", f.GetString("innerResult")+"/tail")
		return nil
	})
	proxy.HandleTask("subgraphentryflow.verify:428/run-tail", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
	...
	t.Run("subgraph_as_first_and_last_node", func(t *testing.T) {
		...
		outcome, err := eng.Run(ctx, "subgraphentryflow.verify:428/outer", nil, nil, nil)
		assert.NoError(err)
