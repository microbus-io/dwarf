...
func TestReducervariantsflow(t *testing.T) {
	...
	graph := workflow.NewGraph("reducervariantsflow.verify:428/reducer")
	...
	proxy.HandleTask("reducervariantsflow.verify:428/task-a", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		return nil
	})
	proxy.HandleTask("reducervariantsflow.verify:428/task-b", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("lo", 5)
		f.SetInt("hi", 5)
	...
	proxy.HandleTask("reducervariantsflow.verify:428/task-c", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("lo", 2)
		f.SetInt("hi", 8)
		f.SetBool("allOk", true)
		f.SetBool("anyOk", false)
	...
	proxy.HandleTask("reducervariantsflow.verify:428/task-d", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
		f.SetInt("lo", 9)
		f.SetInt("hi", 3)
	...
	proxy.HandleTask("reducervariantsflow.verify:428/join", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
