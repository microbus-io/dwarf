...
func superflowSetup(t *testing.T, numShards int) (*engine.Engine, *engine.TestProxy, *visitCounter) {
	...
	for _, name := range []string{"taskA", "taskB", "taskC", "taskD", "taskE", "taskZ", "errorHandler", "subTaskA", "subTaskB"} {
		taskName := name
		proxy.HandleTask("superflow.verify:428/"+kebab(taskName), func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
			return step(ctx, f, taskName)
		})
	}
	proxy.HandleTask("superflow.verify:428/super-sub-call", func(ctx context.Context, f *workflow.Flow, metadata map[string]any) error {
