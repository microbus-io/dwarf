...
| `idx_dwarf_steps_selection` | `(status, parked, priority, fairness_key, created_at, step_id)` + covering `not_before, lease_expires, fairness_weight` (an `INCLUDE` on pgx/mssql; trailing key columns on mysql/sqlite, which have no `INCLUDE`) - partial on pgx/mssql/sqlite, full on mysql | Two-level priority+fairness candidate selection (the three-phase refill: `scanShardBandKeys` -> `planBatch` -> `fetchShardBandSteps`). The `(status, parked)` prefix excludes terminal and parked rows without an in-memory filter; `priority`/`fairness_key` seek the band and its keys. The trailing `(created_at, step_id)` serves the per-key `ROW_NUMBER() OVER (PARTITION BY fairness_key ORDER BY created_at, step_id)` in **both** refill phases: a key's due steps arrive already age-ordered, so the oldest-per-key aggregate (phase 1) and the oldest-N fetch (phase 3) need no sort. `step_id` is listed explicitly because pgx does not implicitly append the PK to a secondary index (mysql/mssql/sqlite do); it also resolves same-`created_at` ties (fan-out siblings can share a millisecond). Cheap to carry: `created_at`/`step_id` are write-once, and the leading `(status, parked)` already repositions the entry on every transition, so the wider key adds size but no extra churn. The `not_before<=NOW AND lease_expires<=NOW` due-predicates and the projected `fairness_weight` are **carried in the index** so phase 1 can evaluate due-ness without deforming the row. All three or none: carrying only the two timestamps leaves `fairness_weight` to drag the row in anyway. They are appended, never part of the ordering prefix, so index order and the no-sort property are unchanged. **Measured under load (single shard, 8 vCPU, PG16, 36k-50k due rows): x1.43 throughput on a fan-out workload, and no measurable change on a linear one** - the latter being the point, since the wider index must not cost anything at a backlog too shallow to need it. **The win is per-tuple scan work, NOT fewer page reads, and that correction matters because it inverts two natural assumptions.** (1) Do not justify this index by buffer counts: the plain `Index Scan` already reads ~0.13 buffers/row in the engine's real insert pattern, because the trailing `(created_at, step_id)` sort keys mean index order tracks physical heap order and consecutive entries land on the same page; the covering plan reads no fewer buffers, it is simply faster per tuple. (An earlier ~1 buffer/row figure came from a fixture that scattered rows and does not describe this table.) (2) The benefit does **not** depend on a warm visibility map: measured with autovacuum having run 0-1 times and ~75% of rows still taking a `Heap Fetch`, the x1.43 held anyway. An index-only scan does want all-visible pages (PostgreSQL's visibility map; InnoDB checks a page-level trx id and needs no VACUUM), so a well-vacuumed steady state should only do better - but the gain is not contingent on getting there, and it degrades to the old plan (never worse) under uniform churn. Cost is storage only: +100% on PostgreSQL, +32% on InnoDB (whose secondary index already carries the PK). `EXPLAIN (ANALYZE, BUFFERS)` should show phase 1 planning as an index-only scan; judge it by scan time rather than by heap-fetch count, which stays high under normal write rates without costing the benefit |
