...
// completing at checkpointBeforeCompleteFlowWrite is a subgraph CHILD, so its completion additionally drives
...
func TestCompleteSurgraph_vs_CancelRoot_BothOrders(t *testing.T) {
	...
	newEngine := func(t *testing.T, prefix string) (e *Engine, url string, callResumed chan struct{}, callBlock chan struct{}) {
	...
	callStatus := func(t *testing.T, e *Engine, fk string) string {
		t.Helper()
		assert := testarossa.For(t)
