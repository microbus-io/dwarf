...
func TestQuery_WorkflowName(t *testing.T) {
	...
	proxy := NewTestProxy()
	alpha := workflow.NewGraph("AlphaFlow", "q.verify:0/alpha")
	alpha.AddTask("A", "q.verify:0/a")
	alpha.AddTransition("A", workflow.END)
	proxy.HandleGraph("q.verify:0/alpha", alpha)
	beta := workflow.NewGraph("BetaFlow", "q.verify:0/beta")
	beta.AddTask("B", "q.verify:0/b")
